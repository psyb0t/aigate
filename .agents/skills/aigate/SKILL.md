...
- **Typed decisions**: `decidealot` at `/decidealot/` (CPU) and `/decidealot-cuda/` (GPU), REST + MCP (`DECIDEALOT=1` / `DECIDEALOT_CUDA=1`). Local Laya and Von models return `choice` / `score` / `noul` answers with probabilities. The caller owns the threshold and the action that follows.
