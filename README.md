...
MCP servers (all optional):
  ...
  └─ piston (via mcp_tools) — execute_code: sandboxed multi-language code execution via nsjail (requires PISTON=1 AND mcp_tools already active per above — hosted by the mcp_tools aggregator, not a standalone MCP server)
```
...
| **[talkies](https://github.com/psyb0t/docker-talkies)** _(optional, `TALKIES=1`)_                              | Unified OpenAI-compatible speech service via `psyb0t/talkies:v0.17.0`. One container, both endpoints: `/v1/audio/transcriptions` and `/v1/audio/speech`. CPU image ships **13 models** — `whisper-large-v3`, `whisper-large-v3-turbo`, `canary-180m-flash`, `nemotron-3.5-asr-0.6b` (NVIDIA Nemotron-3.5-ASR-Streaming via parakeet.cpp, 40+ locales, per-word timestamps), four English Sherpa-ONNX Zipformer variants (`sherpa-zipformer-en-left-64`, `-left-128`, and their `int8` quantized counterparts) `vosk-small-en-us-0.15`, and two phoneme recognizers (`wav2vec2-xlsr-53-espeak`, `zipa-ipa`) that return IPA phones instead of words for ASR, plus `kokoro-82m` (PyTorch) and `kokoro-82m-nvidia` (ONNXRuntime, same voices, no PyTorch on the hot path) for TTS. Stereo channel-split diarization (`diarization=true`), VAD-chunked long audio, idle-unload TTL. Raw Talkies API and live-ASR WebSockets are available at `/talkies/` with the Aigate bearer token. Weights cached in `.data/talkies/`. |
| **[talkies CUDA](https://github.com/psyb0t/docker-talkies)** _(optional, `TALKIES_CUDA=1`)_                    | CUDA-accelerated talkies (`psyb0t/talkies:v0.17.0-cuda`). **22 models** — adds `parakeet-tdt-0.6b-v3`, `canary-1b-flash` (EN/DE/FR/ES + EN↔X translation), `canary-qwen-2.5b` (hybrid SALM), and the full Qwen3-TTS line on top of the CPU set (the Sherpa variants run on the GPU here via an upstream Sherpa CUDA wheel rather than falling back to CPU): `qwen3-tts-0.6b` (Base 0.6B), `qwen3-tts-1.7b` (Base 1.7B), `qwen3-tts-0.6b-custom` / `qwen3-tts-1.7b-custom` (CustomVoice — 9 preset speakers, 1.7B also takes emotion via `instructions`), `qwen3-tts-1.7b-design` (VoiceDesign — NL voice description via `instructions`), plus `chatterbox-turbo` (expressive English TTS — emotion and non-verbal sounds written inline in the text as bracketed tags like `[sigh]` / `[chuckle]`, and transcript-free voice cloning from a reference clip longer than 5 seconds; output carries a neural watermark by default, disable it with `TALKIES_CHATTERBOX_WATERMARK=false`). Raw PCM streaming and the full native API are available at `/talkies-cuda/` only when this profile is enabled; otherwise nginx returns `404`. Drop reference `.wav` files into `${DATA_DIR_TALKIES}/custom-voices/` for Base-mode voice cloning. Kokoro TTS still runs on CPU even inside the CUDA image (fast enough that it doesn't need a GPU). Shares `.data/talkies/`. Requires `nvidia-container-toolkit`. |
...
| **tailscale** _(optional, `TAILSCALE=1`)_                                                                      | Tailscale node running [`tailscale serve`](https://tailscale.com/kb/1242/tailscale-serve). Proxies to nginx on the tailnet only — no public exposure, no port forwarding. Set `TS_AUTHKEY` and `TS_HOSTNAME`, then access aigate at `http://<hostname>.<tailnet>.ts.net` from any tailnet-joined device (plain HTTP — L4 forwarding means Tailscale's hosted HTTPS auto-cert isn't in play). With `CLAUDEBOX=1`, `PIBOX_ZAI=1`, or `PIBOX=1` also on, those agent containers gain outbound tailnet reach (a route plus split DNS) so they can talk to machines on your tailnet. See [docs/services/tailscale.md](docs/services/tailscale.md).                                                                       |
...
Every service uses Docker's `json-file` driver with five 10 MB rotated files,
limiting retained container logs to roughly 50 MB per service. Existing
containers adopt the cap when they are recreated.
