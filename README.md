...
| **tailscale** _(optional, `TAILSCALE=1`)_                                                                      | Tailscale node running [`tailscale serve`](https://tailscale.com/kb/1242/tailscale-serve). Proxies to nginx on the tailnet only — no public exposure, no port forwarding. Set `TS_AUTHKEY` and `TS_HOSTNAME`, then access aigate at `http://<hostname>.<tailnet>.ts.net` from any tailnet-joined device (plain HTTP — L4 forwarding means Tailscale's hosted HTTPS auto-cert isn't in play). With `CLAUDEBOX=1` or `PIBOX_ZAI=1` also on, those agent containers gain outbound tailnet reach (a route plus split DNS) so they can talk to machines on your tailnet. See [docs/services/tailscale.md](docs/services/tailscale.md).                                                                       |
