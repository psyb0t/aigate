...
- **predictalot** _(PREDICTALOT=1 / PREDICTALOT_CUDA=1)_ — per-type model listings (`/v1/timeseries/univariate/models`, `/v1/timeseries/multivariate/models` — verifies non-members are excluded), bearer auth enforcement, univariate forecast via chronos-2, univariate ensemble (`forecast/ensemble`), unknown-model 404, non-member-of-type rejection (timesfm-2.5 on `/v1/timeseries/multivariate/forecast`), the 27-tool MCP surface (`predictalot-forecast_univariate_chronos_2`, `predictalot-list_univariate_models`, `predictalot-unload_models`, etc.), `POST /v1/models/unload`, and a resource manager eviction check: load chronos-2, send a local Ollama embedding through LiteLLM (needs `OLLAMA=1` / `OLLAMA_CUDA=1`), and expect LiteLLM to log that it unloaded chronos-2. Tokens fall back to `AIGATE_TOKEN` when `PREDICTALOT_AUTH_TOKEN` is not set. FM paths use the new `/v1/timeseries/` prefix from upstream v1.0.0; the tabular ML surface added in v1.0.0 is REST-only upstream and not yet exercised by the smoke suite.
- **decidealot** _(DECIDEALOT=1 / DECIDEALOT_CUDA=1)_: open `/health` listing both providers, bearer enforcement (missing and wrong token), the model catalog, a `laya` decision with `choice` and `noul` answers and probabilities, rejection of an unsupported model, a missing model, a `choice` without criteria, and malformed JSON (422) plus a body over 1 MiB (413), direct MCP `initialize` through nginx with a non-local `Host` (the nginx Host pin), the three aggregated MCP tools per variant, and a `list_models` call through the aggregator, which only works while each service name is in its container's `DECIDEALOT_MCP_ALLOWED_HOSTS`, `POST /v1/models/unload`, and a resource manager eviction check: load laya, send a local Ollama embedding through LiteLLM (needs `OLLAMA=1` / `OLLAMA_CUDA=1`), and expect LiteLLM to log that it unloaded laya. Decisions use `laya` only, because a Laya/Von swap is a cold start.
