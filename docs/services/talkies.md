...
External image: [`psyb0t/talkies`](https://github.com/psyb0t/docker-talkies) (pinned to `v0.17.0` / `v0.17.0-cuda`). CPU image ships **13 models** — eleven ASR (`whisper-large-v3`, `whisper-large-v3-turbo`, `canary-180m-flash`, `nemotron-3.5-asr-0.6b` via parakeet.cpp, the four English Sherpa-ONNX Zipformer variants `sherpa-zipformer-en-left-64` / `-left-128` / `-int8-left-64` / `-int8-left-128`, `vosk-small-en-us-0.15`, and two phoneme recognizers `wav2vec2-xlsr-53-espeak` and `zipa-ipa` that emit IPA phones with no lexicon) plus two TTS (`kokoro-82m` PyTorch and `kokoro-82m-nvidia` ONNXRuntime). CUDA image ships **22 models** — adds Parakeet-TDT, Canary-1B-Flash, Canary-Qwen-2.5B SALM, the full Qwen3-TTS line (Base 0.6B + Base 1.7B + CustomVoice 0.6B + CustomVoice 1.7B + VoiceDesign 1.7B), and `chatterbox-turbo` (expressive English TTS with inline emotion tags). The Sherpa variants use the CUDA execution provider in the CUDA image; Kokoro stays CPU-bound in both images.
...
| `local-talkies-wav2vec2-xlsr-53-espeak` | facebook/wav2vec2-xlsr-53-espeak-cv-ft | multilingual | phoneme ASR (v0.17.0). Returns eSpeak IPA phones, no language model or lexicon, so mispronunciations stay as spoken. Both images |
| `local-talkies-zipa-ipa` | anyspeech/zipa-small-crctc-500k | multilingual | phoneme ASR (v0.17.0). IPA via sherpa-onnx offline CTC, 71 MB int8, whole file in one pass. Both images |
